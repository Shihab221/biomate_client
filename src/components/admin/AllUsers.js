  ...
  return (
    <>
      <div className="flex flex-row overflow-hidden">
        ...
        <div className="flex flex-col justify-start pt-4 items-center w-full">
          <div classname="w-full flex justify-end">
            <button
              className={`${classes.addUsersButtonContainer} ${classes.addCourseButton} w-full mb-4 text-center pb-4 px-5 bg-white text-slate-800  hover:bg-slate-900 hover:text-green-400 p-1 border  border-slate-900 rounded-md`}
          ...
          {/* {Object.keys(users).length !== 0 &&
            Object.values(filterItems).map((item, index) => {
              return (
                <div
                  key={Math.random() + 1}
                  className={`w-full flex flex-row md:w-2/12 lg:w-2/12 xl:w-2/12 ${classes.selectFieldsContainer}`}
                >
          ...
          <div
            className={`w-full md:w-10/12 lg:w-10/12 xl:w-9/12 ${classes.tableContainer}`}
          >
            <TableComponents
              rows={rows}
              columns={columns}

...
export const userToken = createAsyncThunk("users/protected", async (_, { getState }) => {
  const token = getState().auth.token; // Assuming the token is stored in Redux state
...
//   name: "auth",
//   initialState: {
//     token: localStorage.getItem("userJwtToken") || null, // Load from localStorage if available
//   },
//   reducers: {
//     logout: (state) => {
//       state.token = null;
//       localStorage.removeItem("userJwtToken"); // Clear token on logout
//     },
//   },
// });
